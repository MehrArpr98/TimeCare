          ...
          <div class="p-3">
            <vee-form @submit="addTask" :validation-schema="schema">
              ...
              <button
                ...
                :disabled="addTask_in_submission"
              >
                <svg
                  v-if="addTask_in_submission"
                  aria-hidden="true"
                  role="status"
...
export default {
  name: 'NewTaskModal',
  components: {},
  props: { showAddModal: Boolean, addToTasks: Function },
  data() {
    return {
      date: null,
      startDate: new Date(),
      ...
      addTask_in_submission: false
    }
  },
  computed: {
    processedDate() {
      if (!this.date) return
      const splitDate = this.date.toString().split(' ')
  ...
  methods: {
    ...
    doNothing() {},
    async addTask(values,{ resetForm }) {
      this.addTask_in_submission = true
      const task = {
      ...
      this.addTask_in_submission = false
      this.closeModal()
      this.addToTasks()
      resetForm();
      this.startDate = new Date();

      this.$swal('Task Added!', '', 'success')
    }
  }
}
</script>
